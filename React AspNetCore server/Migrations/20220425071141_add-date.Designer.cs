// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using React_AspNetCore_server.Context;

#nullable disable

namespace React_AspNetCore_server.Migrations
{
    [DbContext(typeof(PostContext))]
    [Migration("20220425071141_add-date")]
    partial class adddate
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder.HasAnnotation("ProductVersion", "6.0.4");

            modelBuilder.Entity("React_AspNetCore_server.Models.Post", b =>
                {
                    b.Property<int>("PostId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasMaxLength(100000)
                        .HasColumnType("TEXT");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("TEXT");

                    b.HasKey("PostId");

                    b.ToTable("Posts");

                    b.HasData(
                        new
                        {
                            PostId = 1,
                            Content = "This is post 1 and it has some very interesting content. I have also liked the video and subscribed.",
                            Title = "Post 1",
                            UpdatedAt = new DateTime(2022, 4, 25, 15, 11, 41, 509, DateTimeKind.Local).AddTicks(205)
                        },
                        new
                        {
                            PostId = 2,
                            Content = "This is post 2 and it has some very interesting content. I have also liked the video and subscribed.",
                            Title = "Post 2",
                            UpdatedAt = new DateTime(2022, 4, 25, 15, 11, 41, 509, DateTimeKind.Local).AddTicks(216)
                        },
                        new
                        {
                            PostId = 3,
                            Content = "This is post 3 and it has some very interesting content. I have also liked the video and subscribed.",
                            Title = "Post 3",
                            UpdatedAt = new DateTime(2022, 4, 25, 15, 11, 41, 509, DateTimeKind.Local).AddTicks(221)
                        },
                        new
                        {
                            PostId = 4,
                            Content = "This is post 4 and it has some very interesting content. I have also liked the video and subscribed.",
                            Title = "Post 4",
                            UpdatedAt = new DateTime(2022, 4, 25, 15, 11, 41, 509, DateTimeKind.Local).AddTicks(226)
                        },
                        new
                        {
                            PostId = 5,
                            Content = "This is post 5 and it has some very interesting content. I have also liked the video and subscribed.",
                            Title = "Post 5",
                            UpdatedAt = new DateTime(2022, 4, 25, 15, 11, 41, 509, DateTimeKind.Local).AddTicks(231)
                        },
                        new
                        {
                            PostId = 6,
                            Content = "This is post 6 and it has some very interesting content. I have also liked the video and subscribed.",
                            Title = "Post 6",
                            UpdatedAt = new DateTime(2022, 4, 25, 15, 11, 41, 509, DateTimeKind.Local).AddTicks(235)
                        });
                });
#pragma warning restore 612, 618
        }
    }
}
